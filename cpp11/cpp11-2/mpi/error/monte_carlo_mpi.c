...
*/
{
  ...
  int point_max = 2000000; 
  if (argc == 2) {
  		point_max =  atoi(argv[1]);
  }
  int randServer;
...
//    printf ( "  PI is computed to within a tolerance = %f,\n", tolerance );
    printf ( " the number of points examined reaches %d.\n", point_max );
  }
  ...
  printf("%dth of totalTaskNum = %d on %s\n",
				  my_id, numprocs, processorName);
/*
...
*/
      if ( my_id == 0 ) 
      {
       ...
       // done = ( error < tolerance ) || point_max <= ( totalin + totalout );
        done = ( point_max <= ( totalin + totalout ));
   //     printf( "pi = %23.20lf\n", calculatedPi );
