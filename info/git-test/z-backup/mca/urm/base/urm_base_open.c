...
{
    /* set default flags */
    orte_urm_base.active_module = NULL;
    orte_urm_base.allocation_read = false;
    orte_urm_base.total_slots_alloc = 0;

    /* Debugging / verbose output.  Always have stream open, with
        verbose set by the mca open system... */
    orte_urm_base.urm_output = opal_output_open(NULL);
    
    /* Open up all available components */
    if (ORTE_SUCCESS != 
        mca_base_components_open("urm", orte_urm_base.urm_output,
                                 mca_urm_base_static_components,
                                 &orte_urm_base.urm_opened, true)) {
        return ORTE_ERROR;
    }
