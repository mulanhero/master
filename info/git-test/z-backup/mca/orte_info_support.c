...
#include "orte/mca/ufo/base/base.h"//jimmy
#include "orte/mca/ras/base/ras_private.h"
#include "orte/mca/urm/base/base.h"  //jimmy
...
void orte_info_register_types(opal_pointer_array_t *mca_types)
{
    ...
    opal_pointer_array_add(mca_types, "ufo"); //jimmy
                                  ...
                                  opal_pointer_array_t *component_map)
{
    ...
    if (ORTE_SUCCESS != (rc = orte_ufo_base_open()) &&
        ORTE_ERR_BAD_PARAM != rc) {
        str = "ufo_base_open";
        goto error;
    }
    map = OBJ_NEW(opal_info_component_map_t);
    map->type = strdup("ufo");
    map->components = &orte_ufo_base.available_components;
    opal_pointer_array_add(component_map, map);
    if (ORTE_ERR_BAD_PARAM == rc)  {
        str = "ufo";
        goto breakout;
...
//====================jimmy===urm==============
    if (ORTE_SUCCESS != (rc = orte_urm_base_open()) &&
        ...
        map = OBJ_NEW(opal_info_component_map_t);
        map->type = strdup("urm");
        map->components = &orte_urm_base.urm_opened;
        opal_pointer_array_add(component_map, map);
        if (ORTE_ERR_BAD_PARAM == rc)  {
            str = "urm";
            goto breakout;
...
void orte_info_close_components(void)
{
    ...
    (void) orte_ufo_base_close();//jimmy
    (void) orte_ras_base_close();
    (void) orte_urm_base_close();//jimmy
