...
static void odls_hamster_default_wait_local_proc(pid_t pid, int status, void* cbdata);
...
static void orte_odls_yarn_launch_local(int fd, short sd, void *cbdata)
{
    ...
    for (idx=0; idx < orte_local_children->size; idx++) {
        ...
        if (child->name.jobid == jobdat->jobid && child->alive) {
            orte_wait_cb(child->pid, odls_hamster_default_wait_local_proc, NULL);
        }
    }
...
static void odls_hamster_default_wait_local_proc(pid_t pid, int status, void* cbdata)
{
    ...
    //===socket=====start report exitcode

//    int orte_local_jobid = ORTE_LOCAL_JOBID(proc->name.jobid);
//    proc_exitcode_t proc_exitcode_array[1] = {proc->pid, proc->exit_code};
//    orted_ortedservice_ipc_report_proc_exitcode(orte_local_jobid, proc_exitcode_array, 1);
    //===rpc=====start report exitcode
    ReportExitCodeRequestProto request = REPORT_EXIT_CODE_REQUEST_PROTO__INIT;
    ReportExitCodeResponseProto *response;


    int local_jobid = ORTE_LOCAL_JOBID(proc->name.jobid);
    request.localjobid = local_jobid;

    request.n_pid2exitcodes = 1;
    request.pid2exitcodes = xamlloc(sizeof(IntIntMapProto *) * 1);
    request.pid2exitcodes[0] = xmalloc(sizeof(IntIntMapProto));
    int_int_map_proto__init(request.pid2exitcodes[0]);
    (request.pid2exitcodes[0])->has_key = true;
    (request.pid2exitcodes[0])->has_value = true;
    (request.pid2exitcodes[0])->key = proc->pid;
    (request.pid2exitcodes[0])->value = proc->exit_code;

    if (0 != orte_gateway_report_exitcode(&request, &response)) {
    	opal_output(0, "%s odls_hamster_default_wait_local_proc: orte_gateway_report_exitcode failed", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
    }

    if (!request.pid2exitcodes[0]) {
		free(request.pid2exitcodes[0]);
	}

    if (!request.pid2exitcodes) {
		free(request.pid2exitcodes);
	}

    if (!response) {
    	report_exit_code_response_proto__free_unpacked(response, NULL);
	}
    //========end report exitcode
...
//            orted_ortedservice_ipc_report_proc_exitcode(orte_local_jobid, proc_exitcode_array, 1);
            ...
            ReportExitCodeRequestProto request = REPORT_EXIT_CODE_REQUEST_PROTO__INIT;
               ...
               int local_jobid = ORTE_LOCAL_JOBID(child->name.jobid);
               request.localjobid = local_jobid;
               ...
               (request.pid2exitcodes[0])->has_key = true;
               (request.pid2exitcodes[0])->has_value = true;
               (request.pid2exitcodes[0])->key = child->pid;
               (request.pid2exitcodes[0])->value = child->exit_code;
                                        ...
                                        orte_job_t *jobdat)
{
    ...
    //===socket=======report pid start
//    if (pid > 0 && (NULL != child)) {
//    	int orte_local_jobid =  ORTE_LOCAL_JOBID(jobdat->jobid);
//    	int pid_array[1] = { pid };
//    	orted_ortedservice_ipc_report_proc_pid(orte_local_jobid, pid_array, 1);
//    }
    //===rpc=======report pid start
    if (pid > 0 && child) {
		ReportPidRequestProto request = REPORT_PID_REQUEST_PROTO__INIT;
		ReportPidResponseProto *response;
		request.localjobid = ORTE_LOCAL_JOBID(jobdat->jobid);
		request.n_pids = 1;
		request.pids = (int *)xmalloc(sizeof(int) * 1);
		request.pids[0] = pid;

		if (0 != orte_gateway_report_pid(&request, &response)) {
			opal_output(0, "%s odls_default_fork_local_proc: orte_gateway_report_pid failed", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
		}

		if(response) {
			report_pid_response_proto__free_unpacked(response, NULL);
		}
...
static bool check_ready_to_run(int local_jobid) {
	ReadyToRunRequestProto request = READY_TO_RUN_REQUEST_PROTO__INIT;
	ReadyToRunResponseProto *response;
	request.localjobid = local_jobid;

	if (0 != orte_gateway_ready_to_run(&request, &response)) {
		opal_output(0,
				"%s orte_odls_default_launch_local_procs: orte_gateway_ready_to_run failed",
				ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
		goto cleanup;
	}

	bool readyFlag = response->isready;

	if (!response) {
		ready_to_run_response_proto__free_unpacked(response, NULL);
	}
...
int orte_odls_default_launch_local_procs(opal_buffer_t *data)
{
    ...
    //=================socket=========start to check if ready to run
//    int orte_local_jobid = ORTE_LOCAL_JOBID(job);
//
//    while (true) {
//    	bool readytorun = orted_ortedservice_ipc_readytorun(orte_local_jobid);
//    	if(readytorun == true) {
//    		break;
//    	}
//    }
    //=================rpc=========start to check if ready to run
    int local_jobid = ORTE_LOCAL_JOBID(job);

    while (true) {
    	bool readyFlag = check_ready_to_run(local_jobid);
    	if (readyFlag) {
    		break;
    	}
    }
