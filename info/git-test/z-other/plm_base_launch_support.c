...
/*
 * jimmy: declare several variables
 */
static struct timeval daemon_launch_start;
static struct timeval proc_launch_start = {0, 0};

void orte_plm_base_daemons_reported(int fd, short args, void *cbdata)
{
	/*
	 * jimmy: add daemon_launch_end
	 */
	struct timeval daemon_launch_end;
	gettimeofday(&daemon_launch_end, NULL);
	opal_output(0, "############ orte daemon_launch_end.tv_sec = %ld, daemon_launch_end.tv_usec = %ld",
				daemon_launch_end.tv_sec, daemon_launch_end.tv_usec);
	double timecost = ((daemon_launch_end.tv_sec - daemon_launch_start.tv_sec) * 1000000 +
			(daemon_launch_end.tv_usec - daemon_launch_end.tv_usec)) / (double) 1000000;
	opal_output(0, "############ orte timecost_daemon_launch = %f", timecost);
...
{
	/*
	 * jimmy: add daemon_launch_start
	 */
	gettimeofday(&daemon_launch_start, NULL);
    opal_output(0, "############ orte daemon_launch_start.tv_sec = %ld, daemon_launch_start.tv_usec = %ld",
    		daemon_launch_start.tv_sec, daemon_launch_start.tv_usec);
...
void orte_plm_base_launch_apps(int fd, short args, void *cbdata)
{
    ...
    /*
     * jimmy: add proc_launch_start
     */
    gettimeofday(&proc_launch_start, NULL);
    opal_output(0, "############ orte proc_launch_start.tv_sec = %ld, proc_launch_start.tv_usec = %ld",
...
void orte_plm_base_registered(int fd, short args, void *cbdata)
{
    ...
    /*
     * jimmy, add proc_launch_end
     */
    struct timeval proc_launch_end;
    gettimeofday(&proc_launch_end, NULL);
    if (proc_launch_start.tv_sec != 0 || proc_launch_start.tv_usec != 0) {
    	float timecost = ((proc_launch_end.tv_sec - proc_launch_start.tv_sec) * 1000000 +
    				(proc_launch_end.tv_usec - proc_launch_start.tv_usec)) / (float) 1000000;
    	opal_output(0, "############ orte timecost_proc_launch = %f", timecost);
    } else {
    	opal_output(0, "############ hamster_orte proc_launch_end.tv_sec = %ld, proc_launch_end.tv_usec = %ld",
    	    			proc_launch_end.tv_sec, proc_launch_end.tv_usec);
    }
